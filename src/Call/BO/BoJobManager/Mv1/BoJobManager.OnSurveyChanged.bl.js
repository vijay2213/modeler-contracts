    ...
    if (isMatrixSurveyColumn) {
      ...
      if (loSurveyItems.length > 0) {
        ...
        for (var idxSurvey = 0; idxSurvey < loSurveyItems.length; idxSurvey++) {
          ...
          //Do not save "manual added" matrix survey if there exists the same survey question in a job definition list (JDL)
          //getManual of a Survey will be 1 if it is added manually and has no JobDefinitionPKey (=no link to a JDL, usually only for Matrix questions)
          //ProductAddedManually indicates that the survey question is related to a product added manually via AddProduct-Button in the UI
          var isManualSurvey = loSurveyItems[idxSurvey].getManual() === "0" || productList[0].getIsProductAddedManually() == "1";
          if (
            isManualSurvey &&
            Utils.isEmptyString(loSurveyItems[idxSurvey].getJobDefinitionPKey()) &&
            loSurveyItems.length > 1
          ) {
            //Do not save survey because answer is covered via another survey linked to a JDL
