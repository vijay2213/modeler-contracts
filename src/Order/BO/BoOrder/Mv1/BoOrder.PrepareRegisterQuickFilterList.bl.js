...
var addItemsToLoProductQuickFilter = function (items){
  ...
  for (var i=0; i<items.length; i++){
    var groupValue = items[i].getGroupName();
    groupNameExists = loProductQuickFilter.getItemsByParam({"filterValue": groupValue});
    if (groupNameExists.length === 0){
      item = {
        "pKey": PKey.next(),
        //since here the listed filter options are based on (translated) data, the value to use as filter is the same value that is in the product list (while for picklists, this might deviate)
        "text": groupValue,
  ...
  var selectedFilter;
//Reapply filter from either LoProductQuickFilter or QuickFilterCache (selectedFilter is filterValue)
var applySelectedFilterToList = function (){
  var selectedFilterExistsInLo = me.getLoProductQuickFilter().getItemsByParam({"filterValue": selectedFilter});
...
if((ifSelectedInList || ifSelectedInCache) && promotion === "0"){
  ...
  if(ifSelectedInList === true){
    selectedFilter = loProductQuickFilter.getItemsByParam({"isSelected": true})[0].getFilterValue();
  } else{
