...
promise = when.all(deferreds).then(function(){
  ...
  for(x = 0; x < rewards.length; x++) {
    ...
    if(currentReward.getIsReadyToBeApplicable() === "0" && currentReward.getIsApplicable() === "1"){
      ...
      //clear UI checkmark on rewards that are no longer applicable
      //note: selected is intentionally kept as-is so the trim logic
      //in handleRewardSelection can still use it when the reward
      //regains applicability (e.g. on quantity increase)
      currentReward.setIsSelectedOnUI("0");
    }
